...
## Step 10 — Add drag cancellation using multitouch
...
## Step 11 — Add dueDate feature with interactive done visualization. (this commit)
...
- In TaskDelegate.qml, RowLayout is added to lay out the handle and task entry;
...
- In TaskListView.qml, Set `delegateModelAccess: DelegateModel.ReadWrite` so the
  delegate can write the `done` property back through the `DelegateModel` to the
  underlying `TaskItem`.
